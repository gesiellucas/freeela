    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex justify-between items-center">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 bg-brand-500 text-slate-800 rounded-xl text-sm font-bold hover:bg-brand-400 transition-colors shadow-sm"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          ...
          <div key={card.label} className={`bg-white rounded-xl border border-slate-200/60 shadow-card p-5 border-l-4 ${card.color}`}>
            <p className="text-[10px] text-slate-400 uppercase font-black tracking-widest">{card.label}</p>
            <p className="text-2xl font-black mt-1 text-slate-800">{card.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200/60 shadow-card overflow-hidden">
        <table className="w-full text-left">
          <thead className="bg-slate-100/40 text-[10px] font-black uppercase tracking-widest text-slate-400 border-b border-slate-200">
            <tr>
          ...
          <tbody className="divide-y divide-slate-200/60">
            {fiscalNotes.length === 0 ? (
              ...
              fiscalNotes.map(note => {
                ...
                return (
                  <tr key={note.id} className="hover:bg-slate-100/30 transition-colors group">
                    <td className="px-4 py-3 font-mono text-xs font-bold text-slate-700">
                      {note.nf_number || <span className="text-slate-500">—</span>}
                    ...
                    <td className="px-4 py-3 text-xs font-bold text-right text-slate-700">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200/60 rounded-2xl shadow-elevated w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-slate-200/60">
              <h3 className="font-black text-lg text-slate-800">Emitir Nota Fiscal</h3>
            </div>
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-black uppercase text-slate-400 block mb-1">Número NF</label>
                  <input type="text" className="w-full bg-slate-100/60 border border-slate-300/60 rounded-xl p-3 text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-slate-400 transition-all" placeholder="Ex: 000123" value={form.nf_number} onChange={e => setForm({ ...form, nf_number: e.target.value })} />
                </div>
                <div>
                  <label className="text-[10px] font-black uppercase text-slate-400 block mb-1">Série</label>
                  <input type="text" className="w-full bg-slate-100/60 border border-slate-300/60 rounded-xl p-3 text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-slate-400 transition-all" placeholder="Ex: A" value={form.nf_series} onChange={e => setForm({ ...form, nf_series: e.target.value })} />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="date" className="w-full bg-slate-100/60 border border-slate-300/60 rounded-xl p-3 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-slate-400 transition-all" value={form.issue_date} onChange={e => setForm({ ...form, issue_date: e.target.value })} />
                </div>
                <div>
                  <label className="text-[10px] font-black uppercase text-slate-400 block mb-1">Projeto (opcional)</label>
                  <select className="w-full bg-slate-100/60 border border-slate-300/60 rounded-xl p-3 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-slate-400 transition-all" value={form.project_id} onChange={e => setForm({ ...form, project_id: e.target.value })}>
              ...
              <div>
                ...
                <textarea className="w-full bg-slate-100/60 border border-slate-300/60 rounded-xl p-3 text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-slate-400 transition-all h-20 resize-none" placeholder="Descreva o serviço prestado..." value={form.service_desc} onChange={e => setForm({ ...form, service_desc: e.target.value })} />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-black uppercase text-slate-400 block mb-1">Valor Bruto R$ *</label>
                  <input type="number" min="0" step="0.01" className="w-full bg-slate-100/60 border border-slate-300/60 rounded-xl p-3 text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-slate-400 transition-all" placeholder="0,00" value={form.gross_value} onChange={e => setForm({ ...form, gross_value: e.target.value })} />
                </div>
                <div>
                  <label className="text-[10px] font-black uppercase text-slate-400 block mb-1">Alíquota ISS %</label>
                  <input type="number" min="0" max="100" step="0.01" className="w-full bg-slate-100/60 border border-slate-300/60 rounded-xl p-3 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-slate-400 transition-all" value={form.iss_rate} onChange={e => setForm({ ...form, iss_rate: e.target.value })} />
                </div>
              </div>

              {grossVal > 0 && (
                <div className="bg-slate-100/40 rounded-xl p-4 grid grid-cols-3 gap-4 text-center">
                  <div><p className="text-[10px] text-slate-400 uppercase font-black">ISS</p><p className="font-black text-amber-400">R$ {issAmt.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p></div>
                  <div><p className="text-[10px] text-slate-400 uppercase font-black">Valor Líquido</p><p className="font-black text-emerald-400">R$ {netVal.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p></div>
                  <div><p className="text-[10px] text-slate-400 uppercase font-black">Status</p>
                    <select className="mt-1 bg-slate-100 border border-slate-300 rounded-lg text-xs p-1 font-bold text-slate-700" value={form.status} onChange={e => setForm({ ...form, status: e.target.value })}>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" className="w-full bg-slate-100/60 border border-slate-300/60 rounded-xl p-3 text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-slate-400 transition-all" placeholder="Nome ou Razão Social" value={form.recipient_name} onChange={e => setForm({ ...form, recipient_name: e.target.value })} />
                </div>
                <div>
                  <label className="text-[10px] font-black uppercase text-slate-400 block mb-1">CNPJ / CPF do Tomador</label>
                  <input type="text" className="w-full bg-slate-100/60 border border-slate-300/60 rounded-xl p-3 text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-slate-400 transition-all" placeholder="00.000.000/0000-00" value={form.recipient_cnpj} onChange={e => setForm({ ...form, recipient_cnpj: e.target.value })} />
                </div>
              </div>

              <div>
                <label className="text-[10px] font-black uppercase text-slate-400 block mb-1">Observações</label>
                <textarea className="w-full bg-slate-100/60 border border-slate-300/60 rounded-xl p-3 text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-slate-400 transition-all h-16 resize-none" value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} />
              </div>
            </div>
            <div className="p-6 border-t border-slate-200/60 flex justify-end gap-3 bg-slate-50/50">
              <button onClick={() => setIsModalOpen(false)} className="px-4 py-2 border border-slate-300 text-slate-600 rounded-xl text-sm font-bold hover:bg-slate-100 transition-colors">Cancelar</button>
              <button onClick={handleSave} disabled={saving || !form.issue_date || !form.service_desc || !form.gross_value} className="px-4 py-2 bg-brand-500 text-slate-800 rounded-xl text-sm font-bold hover:bg-brand-400 disabled:opacity-50 transition-colors flex items-center gap-2">
