...
const WorkflowBadge = ({ step }) => (
  <span className={`px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider border ${WORKFLOW_STEP_COLOR[step] || 'bg-slate-100 text-slate-500 border-slate-200'}`}>
  ...
  <div className="flex gap-3 group">
    ...
    <div className="flex-1 min-w-0">
      <div className="flex items-center justify-between gap-2 mb-0.5">
        <div className="flex items-center gap-2">
          <span className="text-xs font-semibold text-slate-700">{comment.user?.full_name || 'Você'}</span>
          {projectTitle && (
            <span className="text-[10px] text-slate-400">em <span className="font-medium text-slate-500">{projectTitle}</span></span>
          )}
        </div>
    ...
    {onDelete && (
      <button
        ...
        className="opacity-0 group-hover:opacity-100 transition-opacity text-slate-300 hover:text-red-400 flex-shrink-0 mt-1"
...
const ProjectCard = ({ project, expanded, comments, loadingComments, newComment, onNewCommentChange, onToggle, onAddComment, onDeleteComment }) => {
  const colorDot = PROJECT_COLORS[project.color] || 'bg-slate-300';
  ...
  return (
    <div className={`bg-white rounded-2xl border transition-all duration-200 overflow-hidden ${expanded ? 'border-brand-300 shadow-brand' : 'border-slate-200/60 shadow-card hover:border-slate-300'}`}>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <p className="font-semibold text-slate-800 text-sm truncate">{project.title}</p>
              <p className="text-xs text-slate-400 truncate">{project.client?.name || 'Cliente não vinculado'}</p>
            </div>
      ...
      {expanded && (
        <div className="border-t border-slate-100 px-5 py-4 space-y-4">
          ...
          <div className="flex gap-3">
            ...
            <div className="flex-1 relative">
              <textarea
                ...
                rows={2}
                className="w-full bg-slate-50 border border-slate-200 focus:border-brand-400 focus:ring-2 focus:ring-brand-500/15 rounded-xl px-4 py-2.5 text-sm text-slate-700 placeholder-slate-400 outline-none transition-all resize-none"
              />
              <button
                onClick={() => onAddComment(project.id)}
                disabled={!newComment?.trim()}
                className="absolute right-3 bottom-3 text-brand-500 hover:text-brand-600 disabled:text-slate-300 transition-colors"
          ...
          {loadingComments ? (
            ...
            <p className="text-xs text-slate-400 text-center py-3">Nenhum comentário ainda. Seja o primeiro!</p>
          ) : (
...
const CommentTimeline = ({ comments, loading }) => (
  <div className="bg-white rounded-2xl border border-slate-200/60 shadow-card sticky top-20">
    ...
    <div className="p-4 max-h-[calc(100vh-220px)] overflow-y-auto">
      {loading ? (
        ...
        <div className="text-center py-8">
          ...
          <p className="text-xs text-slate-400 mt-1">Expanda um projeto para começar.</p>
        </div>
      ) : (
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex gap-3">
          <div className="bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-center shadow-card">
            <p className="text-xl font-bold text-slate-800">{activeProjects.length}</p>
            <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">Em aberto</p>
          </div>
          <div className="bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-center shadow-card">
            <p className="text-xl font-bold text-brand-500">{totalComments}</p>
            <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">Comentários</p>
          </div>
        </div>
      </div>
      ...
      {activeProjects.length === 0 ? (
        <div className="bg-white rounded-2xl border border-slate-200/60 shadow-card flex flex-col items-center justify-center py-20 gap-4">
          <div className="w-14 h-14 rounded-2xl bg-slate-100 flex items-center justify-center">
            <FolderOpen size={26} className="text-slate-300" />
          </div>
          <div className="text-center">
            <p className="font-semibold text-slate-600">Nenhum freela em aberto</p>
            <p className="text-sm text-slate-400 mt-1">Converta um lead em projeto para acompanhar aqui.</p>
          </div>
        </div>
      ) : (
