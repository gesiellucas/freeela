...
const STEP_META = {
  focus:      { label: 'Foco',         shortLabel: 'F',  icon: Brain,  ring: 'stroke-amber-400',  bg: 'bg-amber-500',   text: 'text-amber-400',  badge: 'bg-amber-500/10 text-amber-400 border-amber-500/20',  btn: 'bg-amber-500 hover:bg-amber-400 text-zinc-900',  shadow: 'shadow-amber-500/20'  },
  shortBreak: { label: 'Pausa Curta',  shortLabel: 'PC', icon: Coffee, ring: 'stroke-emerald-400', bg: 'bg-emerald-500', text: 'text-emerald-400', badge: 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20', btn: 'bg-emerald-500 hover:bg-emerald-400 text-white', shadow: 'shadow-emerald-500/20' },
  longBreak:  { label: 'Pausa Longa',  shortLabel: 'PL', icon: Coffee, ring: 'stroke-sky-400',     bg: 'bg-sky-500',     text: 'text-sky-400',    badge: 'bg-sky-500/10 text-sky-400 border-sky-500/20',         btn: 'bg-sky-500 hover:bg-sky-400 text-white',         shadow: 'shadow-sky-500/20'    },
};
          ...
          return (
            <div key={i} className="flex flex-col items-center gap-1">
              ...
              <span className={`text-[10px] font-bold ${
                isCurrent ? 'text-zinc-300' : isDone ? 'text-zinc-500' : 'text-zinc-700'
              }`}>
                {cycleNum}
              </span>
      ...
      <div className="flex items-center justify-center gap-4 mt-3">
        {CYCLE_STEPS.map(step => (
          <div key={step} className="flex items-center gap-1.5">
            ...
            <span className="text-[10px] text-zinc-600">{STEP_META[step].label}</span>
          </div>
        ))}
      </div>
    </div>
...
function AlarmModal({ step, currentCycle, totalCycles, onDismiss, onStartNext }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-sm">
      <div className="relative bg-[#18181b] border border-zinc-700 rounded-2xl shadow-2xl p-8 max-w-sm w-full mx-4 text-center">
        <button onClick={onDismiss} className="absolute top-4 right-4 text-zinc-500 hover:text-zinc-300 transition-colors">
          <X size={16} />
        </button>
        ...
        <div className="flex gap-3">
          <button
            onClick={onDismiss}
            className="flex-1 px-4 py-2.5 rounded-xl border border-zinc-700 text-zinc-300 hover:bg-zinc-800 transition-all text-sm font-medium"
          >
              ...
              className={`flex-1 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all shadow-lg ${
                step === 'focus'
                  ...
                  : step === 'shortBreak'
                    ...
                    : 'bg-amber-500 hover:bg-amber-400 text-zinc-900 shadow-amber-500/20'
              }`}
            >
              {nextLabel}
            </button>
...
function ConfigPanel({ config, onSave, onClose }) {
  ...
  const StepConfig = ({ stepKey, label, min, max }) => (
    <div className="flex items-center gap-3 py-3 border-b border-zinc-800 last:border-0">
      <div className={`w-2.5 h-2.5 rounded-sm flex-shrink-0 ${STEP_META[stepKey]?.bg ?? 'bg-zinc-600'}`} />
      <span className="text-sm text-zinc-300 flex-1">{label}</span>
      <div className="flex items-center gap-2">
        <button
          onClick={() => set(stepKey, Math.max(min, local[stepKey] - 1))}
          className="w-7 h-7 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-zinc-400 hover:text-zinc-100 flex items-center justify-center transition-all"
        >
          <Minus size={12} />
        </button>
        <span className="w-12 text-center text-sm font-mono font-semibold text-zinc-100">
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-[#18181b] border border-zinc-700 rounded-2xl shadow-2xl p-6 max-w-sm w-full mx-4">
        {/* Header */}
        <div className="flex items-center justify-between mb-1">
          <h3 className="font-bold text-zinc-100">Configurar Ciclos</h3>
          <button onClick={onClose} className="text-zinc-500 hover:text-zinc-300 transition-colors"><X size={16} /></button>
        </div>
        <p className="text-zinc-500 text-xs mb-5">Cada ciclo = Foco → Pausa Curta → Pausa Longa</p>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 mb-5">
          <div className="flex items-center gap-3">
            <div className="w-2.5 h-2.5 rounded-sm bg-zinc-500 flex-shrink-0" />
            <span className="text-sm text-zinc-300 flex-1">Número de ciclos</span>
            <div className="flex items-center gap-2">
              <button
              ...
              <span className="w-12 text-center text-sm font-mono font-bold text-zinc-100">
                {local.totalCycles}×
              </span>
              <button
        ...
        <div className="mb-5">
          <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-600 mb-2">Preview — 1 ciclo</p>
          <div className="flex items-center gap-2">
        ...
        {/* Resumo total */}
        <div className="bg-zinc-900/60 border border-zinc-800 rounded-xl px-4 py-3 mb-5 flex items-center justify-between">
          <span className="text-xs text-zinc-500">Tempo total estimado</span>
          <span className="text-sm font-bold text-zinc-200 font-mono">{formatMinutes(totalMin)}</span>
        </div>

        <button
          onClick={() => { onSave(local); onClose(); }}
          className="w-full px-4 py-2.5 bg-amber-500 hover:bg-amber-400 text-zinc-900 font-bold rounded-xl transition-all text-sm"
        >
          Salvar configuração
        </button>
      </div>
...
export default function PomodoroView() {
  ...
  return (
    <div className="min-h-[calc(100vh-65px)] bg-zinc-950 flex flex-col pb-10">
      ...
      <div className="flex items-center justify-between px-8 pt-8 pb-6">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-zinc-400 hover:text-zinc-200 hover:bg-white/5 transition-all text-xs font-medium border border-zinc-800"
          >
            <Settings size={13} />
            Configurar
          </button>
      ...
      <div className="flex flex-col items-center px-8">
            ...
            return (
              <React.Fragment key={step}>
                <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium transition-all ${
                  isDone
                    ? `${STEP_META[step].bg} text-zinc-900 opacity-60`
                    : isCurrent
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="p-3 rounded-2xl bg-zinc-900 hover:bg-zinc-800 text-zinc-500 hover:text-zinc-200 transition-all border border-zinc-800"
          >
            <RotateCcw size={17} />
          </button>
          <button
        ...
        {history.length > 0 && (
          <div className="mt-10 w-full max-w-md">
            <h3 className="text-[10px] font-bold uppercase tracking-widest text-zinc-700 mb-3">Histórico</h3>
            <div className="space-y-1.5 max-h-44 overflow-y-auto">
              {[...history].reverse().map((entry, i) => (
                <div key={i} className="flex items-center gap-3 px-3 py-2 bg-zinc-900 rounded-xl">
                  <div className={`w-1.5 h-1.5 rounded-sm ${STEP_META[entry.step].bg} ${entry.skipped ? 'opacity-40' : ''}`} />
                  <span className="text-xs text-zinc-400 flex-1">
                    {STEP_META[entry.step].label}
                    <span className="text-zinc-600"> · ciclo {entry.cycle}</span>
                    {entry.skipped && <span className="text-zinc-700"> · pulado</span>}
                  </span>
