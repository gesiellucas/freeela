  ...
  return (
    <div className="space-y-6 animate-in fade-in">
      ...
      <div className="flex justify-between items-start">
        <div>
          <h2 className="text-2xl font-bold text-warm-900 dark:text-warm-900 tracking-tight">Leads</h2>
          <p className="text-sm text-warm-500 mt-0.5">Acompanhe seus potenciais clientes</p>
        </div>
        <button
          onClick={onAddLead}
        ...
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-zinc-50 dark:divide-warm-300">
            {filtered.length === 0 ? (
                ...
                <td colSpan="6" className="px-6 py-20 text-center">
                  <div className="flex flex-col items-center gap-3">
                    <div className="w-12 h-12 rounded-2xl bg-warm-200 dark:bg-warm-200 flex items-center justify-center">
                      <Users className="text-warm-600 dark:text-warm-500" size={20} />
                    </div>
              ...
              filtered.map(lead => (
                <tr key={lead.id} className="hover:bg-warm-100/80 dark:hover:bg-warm-200/30 transition-colors group">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-sm text-warm-900 dark:text-warm-900">{lead.name}</p>
                        <p className="text-[11px] text-warm-500 mt-0.5">{lead.email}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4 text-sm text-warm-500 dark:text-warm-500 max-w-xs">
                    <p className="truncate">{lead.demand}</p>
                  </td>
                  <td className="px-6 py-4 text-sm font-semibold font-mono text-warm-900 dark:text-warm-900">
                    R$ {(lead.estimated_value || 0).toLocaleString('pt-BR')}
                  </td>
                  <td className="px-6 py-4">
                    <Badge color={statusColor(lead.status)}>{statusLabel(lead.status)}</Badge>
                  </td>
                  <td className="px-6 py-4 text-xs text-warm-500 font-mono tabular-nums">
                    ...
                    {filter === 'active' ? (
                      <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                          ...
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-brand-500 text-warm-900 rounded-lg text-[11px] font-semibold hover:bg-brand-400 transition-colors"
                        >
                      ...
                      <div className="text-right">
                        {lead.metadata?.decline_reason && (
                          <p className="text-[10px] text-warm-500 italic max-w-[200px] ml-auto truncate" title={lead.metadata.decline_reason}>
                            "{lead.metadata.decline_reason}"
                          </p>
                        )}
                      </div>
