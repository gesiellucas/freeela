...
const Badge = ({ color = 'slate', children }) => {
  const colors = {
    ...
    slate: 'bg-zinc-800/60 text-zinc-400 ',
  };
  return (
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      {contracts.length === 0 ? (
        <div className="text-center py-20">
          <FileSignature className="mx-auto text-zinc-700 mb-3" size={40} />
          <p className="text-zinc-500 italic">Nenhum contrato cadastrado.</p>
        ...
        <div className={`grid gap-6 ${syncedSelected ? 'grid-cols-1 lg:grid-cols-5' : 'grid-cols-1'}`}>
              ...
              return (
                <div key={projectId} className="space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    <h3 className="text-xs font-black text-zinc-500 dark:text-zinc-500 truncate">
                      {project?.title || 'Projeto'}
                    </h3>
                    {project?.client?.name && (
                      <span className="text-[10px] text-zinc-500">({project.client.name})</span>
                    )}
                      ...
                      <div
                        ...
                        className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${
                          isSelected
                            ...
                            : 'border-zinc-800/60 bg-zinc-900 hover:border-zinc-700 hover:bg-zinc-800/50'
                        }`}
                      >
                        <FileSignature size={14} className={isSelected ? 'text-blue-500' : 'text-zinc-500'} />
          ...
          {syncedSelected && (
            <div className="lg:col-span-3 space-y-4">
              ...
              <div className="bg-zinc-900 rounded-xl border border-zinc-800/60 shadow-sm p-5">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <button
                    ...
                    className="p-1.5 hover:bg-zinc-800/60  rounded-lg flex-shrink-0"
                  >
                ...
                <div className="grid grid-cols-3 gap-3 mb-4">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full bg-zinc-800/60  border-none rounded-lg px-2 py-1.5 text-xs font-bold cursor-pointer"
                    >
                  ...
                  <div>
                    <p className="text-[10px] font-black uppercase text-zinc-500 mb-1">Início</p>
                    <p className="text-xs font-medium text-zinc-200 py-1.5">
                  ...
                  <div>
                    <p className="text-[10px] font-black uppercase text-zinc-500 mb-1">Validade</p>
                    <p className="text-xs font-medium text-zinc-200 py-1.5">
                ...
                {syncedSelected.description && (
                  <p className="text-xs text-zinc-400 dark:text-zinc-400 mb-2">{syncedSelected.description}</p>
                )}
                {syncedSelected.notes && (
                  <p className="text-xs text-zinc-500 italic">{syncedSelected.notes}</p>
                )}
              </div>
          ...
          <div className="bg-zinc-900 rounded-2xl shadow-2xl w-full max-w-lg">
            <div className="p-6 border-b border-zinc-800/60 flex items-center justify-between">
              <div>
                ...
                <p className="text-[10px] text-zinc-500 mt-0.5">Etapa 1 de 2 — Dados do contrato</p>
              </div>
            ...
            <div className="p-6 space-y-4 max-h-[65vh] overflow-y-auto">
              <div>
                <label className="text-[10px] font-black uppercase text-zinc-500 block mb-1">Título *</label>
                <input type="text" className="w-full bg-zinc-800/60 border-none rounded-xl p-3 text-sm focus:ring-2 focus:ring-blue-500" placeholder="Ex: Contrato de Prestação de Serviços" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} />
              </div>
              <div>
                <label className="text-[10px] font-black uppercase text-zinc-500 block mb-1">Projeto *</label>
                <select className="w-full bg-zinc-800/60 border-none rounded-xl p-3 text-sm focus:ring-2 focus:ring-blue-500" value={form.project_id} onChange={e => setForm({ ...form, project_id: e.target.value })}>
                  <option value="">Selecione um projeto</option>
                  {projects.map(p => <option key={p.id} value={p.id}>{p.title} — {p.client?.name} ({p.status})</option>)}
                </select>
              </div>
              <div>
                <label className="text-[10px] font-black uppercase text-zinc-500 block mb-1">Descrição</label>
                <textarea className="w-full bg-zinc-800/60 border-none rounded-xl p-3 text-sm focus:ring-2 focus:ring-blue-500 h-16" value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} />
              </div>
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <label className="text-[10px] font-black uppercase text-zinc-500 block mb-1">Status</label>
                  <select className="w-full bg-zinc-800/60 border-none rounded-xl p-3 text-sm focus:ring-2 focus:ring-blue-500" value={form.status} onChange={e => setForm({ ...form, status: e.target.value })}>
                    {['draft', 'sent', 'signed', 'cancelled'].map(s => <option key={s} value={s}>{statusLabel(s)}</option>)}
                  </select>
                </div>
                <div>
                  <label className="text-[10px] font-black uppercase text-zinc-500 block mb-1">Data Início</label>
                  <input type="date" className="w-full bg-zinc-800/60 border-none rounded-xl p-3 text-sm focus:ring-2 focus:ring-blue-500" value={form.effective_date} onChange={e => setForm({ ...form, effective_date: e.target.value })} />
                </div>
                <div>
                  <label className="text-[10px] font-black uppercase text-zinc-500 block mb-1">Validade</label>
                  <input type="date" className="w-full bg-zinc-800/60 border-none rounded-xl p-3 text-sm focus:ring-2 focus:ring-blue-500" value={form.expiry_date} onChange={e => setForm({ ...form, expiry_date: e.target.value })} />
                </div>
              </div>
              <div>
                <label className="text-[10px] font-black uppercase text-zinc-500 block mb-1">Observações</label>
                <textarea className="w-full bg-zinc-800/60 border-none rounded-xl p-3 text-sm focus:ring-2 focus:ring-blue-500 h-16" value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} />
              </div>
            </div>
            <div className="p-6 border-t border-zinc-800/60 flex justify-end gap-3">
              <button onClick={handleCloseModal} className="px-4 py-2 border border-zinc-700 text-zinc-400 rounded-xl text-sm font-bold hover:bg-zinc-800/50 transition-colors">Cancelar</button>
          ...
          <div className="bg-zinc-900 rounded-2xl shadow-2xl w-full max-w-lg">
            <div className="p-6 border-b border-zinc-800/60 flex items-center justify-between">
              <div>
                ...
                <p className="text-[10px] text-zinc-500 mt-0.5">Etapa 2 de 2 — Anexar arquivos</p>
              </div>
              <div className="flex gap-1">
            ...
            <div className="p-6 space-y-4">
              <div className="bg-zinc-800/40 rounded-xl p-3">
                <p className="text-xs font-bold text-zinc-200">{createdContract.title}</p>
                <p className="text-[10px] text-zinc-500 mt-0.5">{createdContract.project?.title || 'Projeto'} · {statusLabel(createdContract.status)}</p>
              </div>
              <div>
            ...
            <div className="p-6 border-t border-zinc-800/60 flex justify-end">
              <button
