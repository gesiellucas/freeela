...
const Badge = ({ color = 'slate', children }) => {
  const colors = {
    ...
    slate: 'bg-zinc-800/60 text-zinc-400 text-zinc-300',
  };
  return (
...
export default function PropostasView({
  ...
  const ProposalCard = ({ proposal }) => {
    ...
    return (
      <div className="bg-zinc-900 rounded-xl border border-zinc-800/60 shadow-sm overflow-hidden">
        <div
        ...
        >
          ...
          <div className="flex items-center gap-2 ml-3">
            <select
              ...
              className="text-[10px] border border-slate-200 dark:border-slate-600 rounded-lg px-2 py-1 bg-transparent font-bold cursor-pointer"
            >
        ...
        {isExpanded && (
          <div className="border-t border-slate-100 dark:border-slate-700 p-4 space-y-4">
            {proposal.description && (
              <p className="text-xs text-zinc-400 dark:text-zinc-300">{proposal.description}</p>
            )}
            {proposal.notes && (
              <p className="text-xs text-zinc-500 italic">{proposal.notes}</p>
            )}
            <FileUploader
              userId={userId}
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      {proposals.length === 0 && (
        <div className="text-center py-20">
          ...
          <p className="text-zinc-500 italic">Nenhuma proposta cadastrada.</p>
        ...
        return (
          <div key={projectId} className="space-y-3">
            <div className="flex items-center gap-3">
              ...
              {project?.client?.name && (
                <span className="text-[10px] text-zinc-500 font-medium">({project.client.name})</span>
              )}
              <div className="flex-1 h-px bg-zinc-800/60" />
              <span className="text-[10px] text-zinc-500 font-bold">{projectProposals.length} proposta{projectProposals.length !== 1 ? 's' : ''}</span>
            </div>
      ...
      {ungrouped.length > 0 && (
        <div className="space-y-3">
          <div className="flex items-center gap-3">
            <h3 className="text-sm font-black text-zinc-500">Sem Projeto Vinculado</h3>
            <div className="flex-1 h-px bg-zinc-800/60" />
          </div>
          <div className="space-y-2">
          ...
          <div className="bg-zinc-900 rounded-2xl shadow-2xl w-full max-w-lg">
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <input type="text" className="w-full bg-zinc-800/60 border-none rounded-xl p-3 text-sm focus:ring-2 focus:ring-blue-500" placeholder="Ex: Proposta de Desenvolvimento Web" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} />
              </div>

              <div>
                <label className="text-[10px] font-black uppercase text-zinc-500 block mb-2">Vincular a</label>
                <div className="flex gap-3 mb-3">
                  {[['project', 'Projeto'], ['lead', 'Lead']].map(([val, lbl]) => (
                    <button key={val} onClick={() => setLinkTo(val)}
                      className={`flex-1 py-2 rounded-xl text-xs font-bold border-2 transition-colors ${linkTo === val ? 'border-blue-600 bg-blue-50 text-blue-600 dark:bg-blue-900/20' : 'border-zinc-800/60 text-zinc-500'}`}>
                      {lbl}
                    </button>
                  ))}
                </div>
                {linkTo === 'project' ? (
                  ...
                  <select className="w-full bg-zinc-800/60 border-none rounded-xl p-3 text-sm focus:ring-2 focus:ring-blue-500" value={form.lead_id} onChange={e => setForm({ ...form, lead_id: e.target.value })}>
              ...
              <div>
                ...
                <textarea className="w-full bg-zinc-800/60 border-none rounded-xl p-3 text-sm focus:ring-2 focus:ring-blue-500 h-20" value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} />
              </div>
            ...
            <div className="p-6 border-t border-zinc-800/60 flex justify-end gap-3">
              <button onClick={() => setIsModalOpen(false)} className="px-4 py-2 border border-zinc-700 text-zinc-300 rounded-xl text-sm font-bold hover:bg-zinc-800/50 transition-colors">Cancelar</button>
              <button onClick={handleCreate} disabled={saving || !form.title} className="px-4 py-2 bg-blue-600 text-white rounded-xl text-sm font-bold hover:bg-blue-700 disabled:opacity-50 transition-colors flex items-center gap-2">
