        ...
        {isExpanded && (
          <div className="border-t border-slate-100 dark:border-slate-700 p-4 space-y-4">
            {proposal.description && (
              <p className="text-xs text-slate-600 dark:text-slate-300">{proposal.description}</p>
            )}
            {proposal.notes && (
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      {proposals.length === 0 && (
        <div className="text-center py-20">
          <FileText className="mx-auto text-slate-200 dark:text-slate-700 mb-3" size={40} />
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-lg">
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <input type="text" className="w-full bg-slate-100 dark:bg-slate-800 border-none rounded-xl p-3 text-sm focus:ring-2 focus:ring-blue-500" placeholder="Ex: Proposta de Desenvolvimento Web" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} />
              </div>
              ...
              <div>
                ...
                <div className="flex gap-3 mb-3">
                  {[['project', 'Projeto'], ['lead', 'Lead']].map(([val, lbl]) => (
                    <button key={val} onClick={() => setLinkTo(val)}
                      className={`flex-1 py-2 rounded-xl text-xs font-bold border-2 transition-colors ${linkTo === val ? 'border-blue-600 bg-blue-50 text-blue-600 dark:bg-blue-900/20' : 'border-slate-200 dark:border-slate-700 text-slate-500'}`}>
                      {lbl}
                    </button>
                  ))}
                </div>
                {linkTo === 'project' ? (
                  ...
                  <select className="w-full bg-slate-100 dark:bg-slate-800 border-none rounded-xl p-3 text-sm focus:ring-2 focus:ring-blue-500" value={form.lead_id} onChange={e => setForm({ ...form, lead_id: e.target.value })}>
              ...
              <div>
                ...
                <textarea className="w-full bg-slate-100 dark:bg-slate-800 border-none rounded-xl p-3 text-sm focus:ring-2 focus:ring-blue-500 h-20" value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} />
              </div>
